...
#property description "EMABands_v1 с возможностью изменения таймфрейма во входных параметрах"
...
#define INDICATOR_NAME "EMABands_v1" // константа для имени индикатора
...
//--- инициализация переменных 
   min_rates_total=2;
//--- получение хендла индикатора EMABands_v1
   Ind_Handle=iCustom(Symbol(),TimeFrame,"EMABands_v1",MAPeriod,MAType,Delta);
   if(Ind_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора EMABands_v1");
      return(INIT_FAILED);
     }
...
//---
   if(!CountIndicator(0,NULL,TimeFrame,Ind_Handle,0,UpIndBuffer1,1,DnIndBuffer1,time,rates_total,prev_calculated,min_rates_total)) return(RESET);
   if(!CountIndicator(1,NULL,TimeFrame,Ind_Handle,2,UpIndBuffer2,3,DnIndBuffer2,time,rates_total,prev_calculated,min_rates_total)) return(RESET);
//---     
   return(rates_total);
  }
//+------------------------------------------------------------------+
