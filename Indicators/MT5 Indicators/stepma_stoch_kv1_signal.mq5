//+----------------------------------------------------------------------+
//|                                          StepMA_Stoch_KV1_Signal.mq5 | 
//|                                   Copyright © 2014, Nikolay Kositsin | 
//|                                  Khabarovsk,   farria@mail.redcom.ru | 
//+----------------------------------------------------------------------+
//| For the indicator functioning, place the StepMA_Stoch_KV1.mq5 file   |
...
#define INDTOTAL 1                                   // A constant for the number of displayed indicators
#define INDICATOR_NAME "StepMA_Stoch_KV1"            // A constant for the indicator name
//+----------------------------------------------+
...
#property indicator_color2 clrMagenta,clrMediumVioletRed,clrPlum,clrGray,clrLightSkyBlue,clrTeal,clrDodgerBlue
...
//+------------------------------------------------------------------+   
//| StepMA_Stoch_KV1 indicator initialization function               | 
//+------------------------------------------------------------------+
...
//--- checking correctness of the chart periods
   if(Ind[Number].m_TimeFrame<Period() && Ind[Number].m_TimeFrame!=PERIOD_CURRENT)
     {
      Print("IndInit(",Number,"): The StepMA_Stoch_KV1 indicator chart period cannot be less than the current chart period");
      return(false);
     }
//--- Receiving indicator handles  
   Ind[Number].m_Handle=iCustom(Symbol(),Ind[Number].m_TimeFrame,"StepMA_Stoch_KV1",PeriodWATR,Kwatr,HighLow);
   if(Ind[Number].m_Handle==INVALID_HANDLE)
     {
      Print("IndInit(",Number,"): Failed to get the StepMA_Stoch_KV1 indicator handle");
      return(false);
     }
//---
      ...
      if(Time0>=Time_[0] && Time[bar+1]<Time_[0])
        {
         ...
         double Clr[1];
         //--- copy newly appeared data in the array
         if(CopyBuffer(Ind[Number].m_Handle,1,Time0,1,Clr)<=0) return(RESET);
         
         Ind[Number].m_ColorLineBuffer[bar]=Clr[0];
...
//--- creation of the name to be displayed in a separate sub-window and in a pop up help
   IndicatorSetString(INDICATOR_SHORTNAME,"StepMA_Stoch_KV1("+GetStringTimeframe(Ind[0].m_TimeFrame)+")");
...
//+------------------------------------------------------------------+ 
//| StepMA_Stoch_KV1 iteration function                              | 
//+------------------------------------------------------------------+ 
