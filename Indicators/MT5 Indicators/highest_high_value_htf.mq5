...
#property description "Price maximum for the period specified in inputs"
...
//--- initialization of variables of the start of data calculation    
   ...
   if(ArrayResize(iHigh,level_period)<int(level_period))
     {
      Print("Failed to distribute the memory for the iHigh[] array");
      return(INIT_FAILED);
     }
...
//--- copy newly appeared data in the array
   if(CopyHigh(Symbol(),TimeFrame,level_start,level_period,iHigh)<=0) return(RESET);
//--- search for extremum
   double HH=iHigh[ArrayMaximum(iHigh,0,level_period)];
//--- check the level shift
   SetHline(0,level_name,0,HH,level_color,level_style,level_width,false,level_comment);
//---
   ChartRedraw(0);
   return(rates_total);
  }
