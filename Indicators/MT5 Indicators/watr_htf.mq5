...
#property description "Indicator WATR with timeframe selection option available in input parameters"
...
#define INDICATOR_NAME " WATR"  // A constant for the indicator name
...
//--- initialization of variables 
   min_rates_total=2;
//--- getting the handle of the WATR indicator
   Ind_Handle=iCustom(Symbol(),TimeFrame,"WATR",WATR_K,WATR_M,ATR,0);
   if(Ind_Handle==INVALID_HANDLE)
     {
      Print(" Failed to get the handle of the WATR indicator");
      return(INIT_FAILED);
     }
...
//---
   if(!CountIndicator(0,Symbol(),TimeFrame,Ind_Handle,0,Ind1Buffer,1,Ind2Buffer,2,
      Ind3Buffer,3,Ind4Buffer,time,rates_total,prev_calculated,min_rates_total)) return(RESET);
//---     
   return(rates_total);
  }
//---
