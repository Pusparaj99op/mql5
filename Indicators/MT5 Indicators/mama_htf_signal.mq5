...
//|  Объявление констант                           |
//+------------------------------------------------+
#define INDICATOR_NAME      "MAMA"              // Имя индикатора
...
//+------------------------------------------------+ 
//| Перечисление для расчёта входной цены          |
//+------------------------------------------------+ 
...
//--- получение хендла индикатора MAMASign
   Ind_Handle=iCustom(Symb,Timeframe,"MAMASign",FastLimit,SlowLimit,IPC);
   if(Ind_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора MAMASign");
      return(INIT_FAILED);
     }
//--- инициализации переменной для короткого имени индикатора
   string shortname;
   StringConcatenate(shortname,INDICATOR_NAME,"( ",FastLimit,", ",SlowLimit,", ",EnumToString(IPC)," )");
